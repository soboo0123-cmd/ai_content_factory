...
1.  **위험중립 가치평가(Risk-Neutral Valuation):** 복제 포트폴리오의 개념은 무위험 기대값을 구하고 이를 이자율로 할인하는 형태인 $C = \dfrac{p C_u + (1-p) C_d}{1+r}$로 완벽히 치환됩니다.
...
3.  **이항 분포로의 확장:** 2기간 최종 공식 $C = \dfrac{p^2 C_{uu} + 2p(1-p)C_{ud} + (1-p)^2 C_{dd}}{(1+r)^2}$의 분자는 완벽한 확률 결합계(합이 1)를 이루고 있으며, 이는 다기간 모델이 결국 '이항분포(Binomial Distribution)'의 통계적 법칙으로 수렴해 갈 것임을 예고합니다.
