...
(참고) 주식이 상승 또는 하락한다는 이항분포를 가정했는데 주가의 평균이 이항분포의 평균 공식 $np$가 그대로 나오지 않는 이유는?
구하고자 하는 변수가 '상승 횟수'가 아니라 '로그수익률'이기 때문입니다.
 - 차이점: '상승 횟수' vs '로그수익률'이항분포의 확률변수 ($X$): 상승 시 $1$, 하락 시 $0$을 갖는 상승 횟수입니다. $n=1$일 때 상승 횟수 $X$의 기댓값은 정확히 $np = 1 \cdot q = q$가 됩니다. 로그수익률의 확률변수 ($R$): 상승 시 $1$이 아니라 $\ln u$, 하락 시 $0$이 아니라 $\ln d$라는 실제 수익률 가치를 갖습니다.
