...
초기 주가 $S_0 = 100$달러인 자산이 5단계($N=5$) 동안 상승($u=1.10$) $\to$ 하락($d=0.90$) $\to$ 상승($u=1.10$) $\to$ 상승($u=1.10$) $\to$ 하락($d=0.90$)의 경로를 밟았다고 가정합니다. 곱셈의 교환법칙에 의해 최종 주가 $S_5$는 다음과 같이 계산됩니다.

$$S_5 = 100 \times 1.10 \times 0.90 \times 1.10 \times 1.10 \times 0.90 = 100 \times (1.10)^3 \times (0.90)^2 = 107.811\text{ (달러)}$$

매 단계별 주가 흐름과 이에 대응하는 이산수익률 배수, 그리고 로그수익률을 정리하면 아래와 같습니다.

| 시점 ($t$) | 주가 이동 상태 ($S_t$) | 이산수익률 배수 ($\frac{S_t}{S_{t-1}}$) | 1기간 로그수익률 ($R_t = \ln \frac{S_t}{S_{t-1}}$) | 로그 계산값 |
| --- | --- | --- | --- | --- |
| $t = 0$ | $S_0 = 100.000$ | - | - | - |
...
5기간 동안의 전체 이산수익률은 개별 수익률 배수의 곱에서 1을 뺀 값입니다.

$$\text{누적 이산수익률} = \frac{107.811 - 100}{100} = (1.10 \times 0.90 \times 1.10 \times 1.10 \times 0.90) - 1 = 7.811\%$$

반면 최초 시점 $S_0$ 대비 최종 시점 $S_5$의 총 누적 로그수익률($R_{\text{total}}$)을 구해보면 다음과 같습니다.

$$R_{\text{total}} = \ln\left(\frac{S_5}{S_0}\right) = \ln\left(\frac{107.811}{100.000}\right) = \ln(1.07811) \approx 0.07521$$

이번에는 매 단계별 1기간 로그수익률 $R_1, R_2, R_3, R_4, R_5$를 단순 합산해 보겠습니다.
...
$$\bar{R}_{\text{log}} = \frac{3\ln u + 2\ln d}{5} = \underbrace{\left(\frac{3}{5}\right)}_{\text{상승 상대빈도}} \ln u + \underbrace{\left(\frac{2}{5}\right)}_{\text{하락 상대빈도}} \ln d$$

여기서 시행 횟수 $N \to \infty$로 확장을 해보면, 대수의 법칙에 의해 상대빈도 $\frac{N_u}{N}$는 이론적 상승 확률 $q$로 수렴합니다. 이로써 1기간 로그수익률의 기대값 $E[R_{\text{log}}]$식이 자연스럽게 도출됩니다.

$$E[R_{\text{log}}] = q \ln u + (1-q) \ln d$$

이처럼 로그 가산성(Log Additivity)은 주가의 확률과 평균을 표본경로의 상대빈도 개념에서 이론적 기대값으로 매끄럽게 연결해 주며, 통계적 중심극한정리와 연속시간 금융이론으로 나아가는 핵심 다리가 됩니다.

참고로
이산수익률 관점에서는 10% 상승 후 10% 하락하더라도 원래 주가로 돌아오지 않습니다.($S_0 = 99$)

* **이산수익률의 기대값 (산술기대값)**

$$E[R_{\text{disc}}] = 0.5(0.10) + 0.5(-0.10) = 0\%$$

그런데 이산수익률의 기대값은 1회 시행 시 평균적으로 손익이 0달러인 것처럼 보입니다.

* **로그수익률의 기대값 및 기하평균**

$$E[R_{\text{log}}] = 0.5 \ln(1.10) + 0.5 \ln(0.90) \approx -0.005026$$
$$e^{E[R_{\text{log}}]} = 1.10^{0.5} \times 0.90^{0.5} = \sqrt{0.99} \approx 0.994987$$


실제 복리로 계속 투자할 경우, 회당 약 $-0.5%$씩 자산이 줄어드는 **변동성 드래그(Volatility Drag)** 현상을 반영합니다.
...
(참고) 주식이 상승 또는 하락한다는 이항분포를 가정했는데 주가의 평균이 이항분포의 평균 공식 $np$가 그대로 나오지 않는 이유는 구하고자 하는 변수가 '상승 횟수'가 아니라 '로그수익률'이기 때문입니다.1. 차이점: '상승 횟수' vs '로그수익률'이항분포의 확률변수 ($X$): 상승 시 $1$, 하락 시 $0$을 갖는 상승 횟수입니다.$n=1$일 때 상승 횟수 $X$의 기댓값은 정확히 $np = 1 \cdot q = q$가 됩니다.로그수익률의 확률변수 ($R$): 상승 시 $1$이 아니라 $\ln u$, 하락 시 $0$이 아니라 $\ln d$라는 실제 수익률 가치를 갖습니다.
