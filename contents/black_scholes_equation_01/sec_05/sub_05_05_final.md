...
1. **$d_2$와 누적분포함수의 관계**:
   ...
   ※ 어떤 기준값 $-d_2$는 평균($0$)의 왼쪽에 있는 음수 $(-d_{2}$) 지점부터 시작하여, 오른쪽 끝(양의 무한대)까지 이어지는 거대한 영역의 면적입니다.
...
2. **기초자산 인도 항 (Asset Delivery Part, $S_T \cdot \mathbb{1}$)**:
   만기 주가가 $K$를 넘을 때, 가치가 변동하는 주식 자체($S_T$)를 인도받는 계약입니다. 이 경우 가치 평가는 단순 확률이 아니라 **"주가가 높을 때 더 큰 주식을 받는다"**는 가중치가 반영되어야 합니다. (d<R<u의 관계이므로 q'은 1보다 크다) 
