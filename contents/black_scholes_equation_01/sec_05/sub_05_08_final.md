...
$$P(S_{100} \ge S_0 e^{0.11}) \approx \mathbf{0.453621}$$
...
여기서 우리는 직관에만 의존해 연속성 수정을 누락한 '불완전 근사'와, 0.5의 보정을 가한 '정밀 근사'를 대조하여 수학적 장치의 강력함을 입증하겠습니다.
...
* **경로 A: 연속성 수정을 누락한 불완전 근사 ($z$ 기준값으로 $k=52$ 직접 대입)**:
  ...
  * 정밀 이산 확률(0.453621)과의 절대 오차: **0.039338 (약 3.93%의 구조적 오차 발생)**
...
* **경로 B: 연속성 수정을 적용한 정밀 근사 ($z$ 기준값으로 $k-0.5 = 51.5$ 대입)**:
  ...
  * 정밀 이산 확률(0.453621)과의 절대 오차: **0.000002 (불과 0.0002%의 극소 오차!)**

#### 수치 분석의 수학적 시사점
위 대조 결과는 연속성 수정(Continuity Correction)의 위력을 극명하게 보여줍니다. $n=100$이라는 비교적 유한한 단계에서도, 단지 기준점을 $0.5$ 하향 조정하여 52번째 막대의 면적을 온전히 품어낸 것만으로 오차를 3.93%에서 0.0002%로 혁신적으로 소멸시켰습니다. 

물론 시행 단계 $n$이 $\infty$로 무한히 커짐에 따라 분모인 표준편차 $SD[X] \propto \sqrt{n}$이 무한히 커지므로, 분자의 $0.5$ 차이가 갖는 상대적 영향력($0.5 / \sqrt{n}$)은 궁극적으로 $0$으로 소멸합니다. 즉, 극한의 세계($n \to \infty$)에서는 수정을 가하든 가하지 않든 오차가 소멸하며 동일한 곡선 면적으로 수렴하지만, 현실적인 유한 이산 모형을 다룰 때 이 작고 정교한 연속성 보정은 모형의 신뢰성을 담보하는 필수적인 수학적 징검다리 역할을 수행합니다.
...
| **계산 확률값** | **0.453621** | **0.414283** | **0.453623** |
| **실제 절대 오차** | 기준점 (0.00) | **0.039338 (3.93%)** | **0.000002 (0.0002%)** |
| **$n \to \infty$ 극한** | 곡선 아래 면적으로 수렴 | 오차 0으로 소멸 | 오차 0으로 소멸 |
...
이로써 우리는 8장에 걸친 장대한 여정—**이항 격자 모델의 보폭 설계법, $p$-측도와 $q$-측도의 유도, 미소 시간 격자에서의 왜도 소멸, 그리고 최종적인 표준정규분포 누적함수로의 수렴 검증까지**—을 완벽하게 마무리 지었습니다. 거친 이산 시간의 모형이 어떻게 연속 시간의 매끄러운 흐름과 화해하는지 수학적, 물리적 정합성을 모두 규명해 낸 것입니다.

이제 모든 기초 확률론적 빌딩 블록이 완벽하게 조율되었습니다. 

다음 **9장**에서는 마침내 이 연속 시간 한계 모델들을 총결합하여, 금융공학 역사상 가장 아름다운 방정식이자 금자탑인 **블랙-숄즈(Black-Scholes) 연속 시간 지배 미분 방정식**을 이항 모델의 극한으로부터 유도하는 대장정의 종착역으로 힘차게 나아가겠습니다.
