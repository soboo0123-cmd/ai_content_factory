...
앞에서 구한 델타와 B를 $C_0$에 대입하여 정리하면 다음과 같은 공식을 얻는다. 

$$ C_0 = \left[ \frac{C_u - C_d}{(u - d)S_0} \right] S_0 + \frac{u C_d - d C_u}{(u - d)(1+r)} $$


이것을 $C_u, C_d$에 대해 정리하면 다음과 같다.

$$ C_0 = \frac{1}{1+r} \left[ ( \frac{(1+r)  - d}{u - d} ) C_u + ( \frac{u - (1+r)}{u - d} ) C_d \right] $$
...
내용: 만기 시점 상향($u$), 하향($d$) 두 상태의 연립방정식을 풀어 $\Delta$와 $B$를 구한 뒤 $C_0$ 식에 대입해 묶어줍니다.
한 줄 요약: "계산 과정을 거쳐 식을 $C_u$와 $C_d$ 기준으로 다시 묶으면 다음과 같이 정리됩니다."

$$C_0 = e^{-rT} \left[ p C_u + (1-p) C_d \right], \quad \text{단, } p = \frac{e^{rT} - d}{u - d}$$

### 3.2 C_u와 C_d로 붂어 봤더니

내용: 도출된 계수 $p$의 성질을 관찰합니다.한 줄 요약:$0 < p < 1$이므로 $p$를 마치 '확률'처럼 다룰 수 있습니다.
...
이 $p$를 적용해 주식의 만기 기대값을 구해보면 기대수익률이 정확히 무위험 이자율 $r$이 됩니다 ($E^Q[S_T] = S_0 e^{rT}$).



위험에 중립적이라는 것은 위험에 대한 회피 심리가 없는 것입니다. 수식으로 보면 그 의미가 명확한데
$$
S_0(1+r) = p \cdot uS_0 + (1-p) \cdot dS_0
$$

수식의 의미는 주가 $S_0$ 금액만큼 채권같은 이자 지급 자산에 1기간 투자한 것과
주식은 올라서 이익을 볼 수도, 내려서 손실을 볼 수도 있는데 
상승확률$p$를 잘 선택하면 이 둘을 같게 만들 수 있다는 의미입니다.
...
즉, 복제 포트폴리오라는 '무차익 조건'만 썼을 뿐인데, 결과적으로 **'위험을 전혀 신경 쓰지 않는 가상 세계의 기대값'**을 구하는 것과 동일한 식이 도출된 것입니다."
